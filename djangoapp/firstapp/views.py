...
def user(request):
    if request.method== "POST":
        ...
        form_data = reg_info()
    context = {"form": form}
    return render(request, 'resultget.html', context)

def age_check(request, age):
    formate = "%Y-%m-%d"
    try:
...
def validate_pan(request, pan_num):
    dates = []
    validate_result = []
    for p in request_info.objects.raw('SELECT id,panid,request_date FROM firstapp_request_info'):
        if pan_num == p.panid:
